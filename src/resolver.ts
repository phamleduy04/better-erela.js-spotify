...
export default class resolver {
    ...
    public async makeRequest<T>(endpoint: string): Promise<T | null> {
        try {
            ...
            const req = await request(endpoint.startsWith("http") ? endpoint : `${this.BASE_URL}${endpoint.startsWith("/") ? endpoint : `/${endpoint}`}`, { headers: { Authorization: this.token! } });
            if ([401].includes(req.statusCode)) {
                await this.renew();
                return await this.makeRequest(endpoint);
            }
            return await req.body.json() as Promise<T>;
        } catch {
            return null;
        }
    }
    ...
    public async getToken(): Promise<number> {
        const response = await request("https://accounts.spotify.com/api/token?grant_type=client_credentials", {
            method: "POST",
            headers: {
        ...
        const { access_token, expires_in } = await response.body.json() as ISpotifyAccessTokenAPIResult;
        if (!access_token) {
    ...
    public async fetchAccessToken(): Promise<number> {
        const response = await request("https://open.spotify.com/get_access_token", { headers: { "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.124 Safari/537.36 Edg/91.0.864.59", referer: "https://open.spotify.com" } });
        const { accessToken, accessTokenExpirationTimestampMs } = await response.body.json() as ISpotifyAccessTokenAPIScrapeResult;
